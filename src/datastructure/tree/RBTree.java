...
public class RBTree<T extends Comparable<T>> {
    ...
    //层次遍历时保存各个节点
    RBTree[] elements = new RBTree[max];
    int front;//层次遍历队首
    int rear;//层次遍历队尾
    public T data;
    public RBTree parent, left, right;
    ...
    //构造树
    public RBTree(T data, RBTree left, RBTree right) {
        this.data = data;
        this.left = left;
        this.right = right;
    }
    ...
    //先序遍历
    public void preOrder(RBTree root) {
        if (root == null) {
            return;
        } else {
    ...
    //中序遍历
    public void inOrder(RBTree root) {
        if (root == null) {
            return;
        } else {
            inOrder(root.left);
    ...
    //后序遍历
    public void postOrder(RBTree root) {
        if (root == null) {
            return;
        } else {
            postOrder(root.left);
    ...
    //层次遍历
    public void layerOrder(RBTree root) {
        elements[0] = root;
        front = 0;
        rear = 1;
    ...
    private void fillScreen(Object[][] screen, int row, int columnBegin, int columnEnd, RBTree node) {
    ...
    //插入一个节点
    public boolean insert(RBTree node) {
    ...
    public boolean delete(T key) {
        ...
        if (key.compareTo(this.data) > 0) {
            ...
            if (this.left == null && this.right == null) {
                ...
                RBTree<T> node = this.right;
                this.data = node.data;
                this.left = node.left;
                this.right = node.right;
            } else if (this.right == null) {//只有左子树
                RBTree<T> node = this.left;
                this.data = node.data;
                this.left = node.left;
                this.right = node.right;
            } else {//既有左子树，又有右子树
    ...
    public static void main(String[] args) {
        ...
        tree.insert(new RBTree(1));
        tree.insert(new RBTree(2));
        tree.insert(new RBTree(3));
        tree.insert(new RBTree(4));
        tree.insert(new RBTree(6));
        tree.insert(new RBTree(7));
        tree.insert(new RBTree(8));
        tree.insert(new RBTree(9));
        tree.insert(new RBTree(10));
