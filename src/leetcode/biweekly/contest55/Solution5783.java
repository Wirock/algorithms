 ...
 * @date 2021/6/27
 */
public class Solution5783 {
    class MovieRentingSystem {
        Map<Integer,Map<Integer,int[]>> shopMap = new HashMap<>();
        Set<int[]> rent = new TreeSet<>((x,y)->{
            int p = x[2]-y[2];
            if(p!=0)return p;
            int s = x[0]-y[0];
            if(s!=0)return s;
        ...
        Map<Integer,Set<int[]>> movieMap = new HashMap<>();

        public MovieRentingSystem(int n, int[][] entries) {
            for(int i=0;i<entries.length;i++){
                if(!shopMap.containsKey(entries[i][0]))shopMap.put(entries[i][0],new HashMap<>());
                shopMap.get(entries[i][0]).put(entries[i][1],entries[i]);
                if(!movieMap.containsKey(entries[i][1]))movieMap.put(entries[i][1],new TreeSet<>((x,y)->{
                    int p = x[2]-y[2];
                    if(p!=0)return p;
                    return x[0]-y[0];
                }));
                movieMap.get(entries[i][1]).add(entries[i]);
            }
        }

        public List<Integer> search(int movie) {
            List<Integer> ans = new ArrayList<>();
            Set<int[]> set = movieMap.getOrDefault(movie,new TreeSet<>((x,y)->{
                int p = x[2]-y[2];
                if(p!=0)return p;
                return x[0]-y[0];
            }));
        ...
        public void rent(int shop, int movie) {
            int[] m = shopMap.get(shop).get(movie);
            rent.add(m);
            movieMap.get(movie).remove(m);
        }

        public void drop(int shop, int movie) {
            int[] m = shopMap.get(shop).get(movie);
            movieMap.get(movie).add(m);
            rent.remove(m);
        }
