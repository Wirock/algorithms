...
public class Solution5925 {
    ...
    public int countPyramids(int[][] grid) {
        int m = grid.length;
        int n = grid[0].length;
        int[][] count1 = new int[m][n];//以grid[i][j]为顶点的正金字塔数量
        for(int i=m-2;i>=0;i--){
            for(int j=1;j<n-1;j++){
                if(grid[i][j]==1){
                    if(grid[i+1][j]==1&&grid[i+1][j-1]==1&&grid[i+1][j+1]==1){
                        count1[i][j]=1+Math.min(count1[i+1][j],Math.min(count1[i+1][j-1],count1[i+1][j+1]));
                    }
                }
            }
        }
        int[][] count2 = new int[m][n];//以grid[i][j]为顶点的倒金字塔数量
        for(int i=1;i<m;i++){
            for(int j=1;j<n-1;j++){
                if(grid[i][j]==1){
                    if(grid[i-1][j]==1&&grid[i-1][j-1]==1&&grid[i-1][j+1]==1){
                        count2[i][j]=1+Math.min(count2[i-1][j],Math.min(count2[i-1][j-1],count2[i-1][j+1]));
                    }
                }
            }
        }
