 ...
 * @date 2021/7/11
 */
public class Solution5795 {
    //dp,f(x,y)表示在y时间内到达x城市的最小花费
    //f(x,y) = min(f(x,y)，f(xi,y-yi)+passingFees[x]),xi为不等于x的城市，yi是xi到x的时间
    /*public int minCost(int maxTime, int[][] edges, int[] passingFees) {
        int n = passingFees.length;
        Map<Integer,Map<Integer,Integer>> timeMap = new HashMap<>();
        //注意坑：两个城市间可以有多条道路
        for(int[] e:edges){
           timeMap.putIfAbsent(e[0],new HashMap<>());
           Map<Integer, Integer> map = timeMap.get(e[0]);
           map.put(e[1],Math.min(map.getOrDefault(e[1],Integer.MAX_VALUE),e[2]));
           timeMap.putIfAbsent(e[1],new HashMap<>());
           map = timeMap.get(e[1]);
           map.put(e[0],Math.min(map.getOrDefault(e[0],Integer.MAX_VALUE),e[2]));
        }
        ...
        for(int t=1;t<=maxTime;t++){
            ...
            for(int i=1;i<n;i++){
                dp[i][t] = dp[i][t-1];
                for(Map.Entry<Integer,Integer> entry : timeMap.getOrDefault(i,new HashMap<>()).entrySet()){
                    if(entry.getValue()>0&&t>=entry.getValue()&&dp[entry.getKey()][t-entry.getValue()]>0){
                        dp[i][t] = dp[i][t]<0?dp[entry.getKey()][t-entry.getValue()]+passingFees[i]:Math.min(dp[i][t],dp[entry.getKey()][t-entry.getValue()]+passingFees[i]);
                    }
                }
            }
        }
