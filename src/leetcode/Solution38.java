...
public class Solution38 {
    ...
    static {
        ...
        for(int i=1;i<arr.length;i++){
            ...
            int count = 0;
            char cur = arr[i-1].charAt(0);
            for(int j=0;j<arr[i-1].length();j++){
                if(arr[i-1].charAt(j)==cur){
                    count++;
                }else{
                    sb.append(count).append(arr[i-1].charAt(j-1));
                    count=1;
                    cur=arr[i-1].charAt(j);
                }
            }
            sb.append(count).append(arr[i-1].charAt(arr[i-1].length()-1));
            arr[i] = sb.toString();
        }
    }
